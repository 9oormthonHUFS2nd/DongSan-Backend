...
class UserWalkwayUseCaseTest {
    ...
    class Describe_getUserUploadWalkway{
        ...
        void it_returns_walkway_list(){
            ...
            // then
            Assertions.assertThat(response).hasSameSizeAs(walkways);
            for(int i=0; i<response.size(); i++){
                Walkway walkwayResponse = response.get(i);
                Walkway walkwayInfo = walkways.get(i);
                Assertions.assertThat(walkwayResponse.getId()).isEqualTo(walkwayInfo.getId());
                Assertions.assertThat(walkwayResponse.getName()).isEqualTo(walkwayInfo.getName());
                Assertions.assertThat(walkwayResponse.getHashtagWalkways()).hasSameSizeAs(walkwayInfo.getHashtagWalkways());
            }
        }
    ...
    class Describe_getUserLikedWalkway{
        ...
        void it_returns_walkway_list(){
            ...
            // then
            Assertions.assertThat(response).hasSameSizeAs(likedWalkways);
            for(int i=0; i<response.size(); i++){
                Walkway walkwayResponse = response.get(i);
